...
const tools: ChatCompletionTool[] = [
  ...
  },
  {
    ...
    function: {
      name: "cancel_gather",
      description: "Скасувати ВЕСЬ збір команди цілком. Використовуй коли користувач хоче скасувати/відмінити/отменить весь збір (укр/рос/англ), а НЕ коли хоче вийти зі списку. Перевірка прав відбувається всередині інструменту — просто викликай.",
  ...
  },
  {
    ...
    function: {
      ...
      description: "Змінити час збору. Використовуй коли користувач просить перенести/змінити час будь-якою мовою (укр/рос/англ): перенеси, перенос, змінити, поміняти, измени, измените, поменять, перенеси, давай о, зміни на, change to. Перевірка прав відбувається всередині інструменту — просто викликай.",
      parameters: {
        type: "object",
        properties: {
          new_time: {
  ...
  },
  {
    ...
    function: {
      name: "add_player",
      description: "Додати ІНШОГО гравця до збору за username. Використовуй коли просять додати когось конкретного: додай @user, +@user, запиши @user. Перевірка прав відбувається всередині інструменту — просто викликай.",
      parameters: {
        type: "object",
        properties: {
          username: {
  ...
  },
  {
    ...
    function: {
      name: "remove_player",
      description: "Видалити ІНШОГО гравця зі збору за username. Використовуй коли просять прибрати/видалити когось: прибери @user, -@user, видали @user. Перевірка прав відбувається всередині інструменту — просто викликай.",
      parameters: {
        type: "object",
        properties: {
          username: {
...
- cancel_gather — скасувати збір.
- update_gather_time — змінити/перенести час.
...
- ТИ НЕ ЗНАЄШ хто автор збору і НЕ МУСИШ це визначати. Якщо інструмент поверне помилку прав — тоді скажеш юзеру.
- НІКОЛИ не відмовляй заранее і НЕ ПИТАЙ підтвердження "ти точно автор?". Якщо просять змінити час, скасувати, додати/видалити гравця — ОДРАЗУ викликай відповідний інструмент. Бекенд сам зрозуміє чи можна.
- Зрозумій намір з будь-якої мови (укр/рос/англ): "измени на 22:30", "перенеси на 22:00", "change to 21:00" — все це update_gather_time. "отмена", "скасуй", "cancel" — це cancel_gather. І т.д.

ГОЛОВНЕ ПРАВИЛО: Якщо є відповідний інструмент — використай його ОДРАЗУ. НІКОЛИ не кажи "використай команду /gather" чи "якщо ти автор — дай знак". Просто викликай тул.
